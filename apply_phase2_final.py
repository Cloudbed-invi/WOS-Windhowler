...
old_block = r'''                # AI Curve Sanity Check \(Power Law: 844\.19 \* L\^4\.0878\)\s+start = 844\.19 \* \(lvl \*\* 4\.0878\)\s+next_start = 844\.19 \* \(\(lvl \+ 1\) \*\* 4\.0878\)\s+expected = start \+ \(pct / 100\.0\) \* \(next_start - start\)\s+error_margin = abs\(dmg - expected\) / expected\s+item = \{\s+"Level": lvl, \s+"Percent": pct, \s+"Damage": dmg, \s+"Deviation": f"\{error_margin \* 100:\.1f\}%",\s+"Timestamp": row\['Timestamp'\]\s+\}\s+# 15% tolerance for new submissions\s+if error_margin <= 0\.15:\s+valid\.append\(item\)\s+else:\s+flagged\.append\(item\)'''
